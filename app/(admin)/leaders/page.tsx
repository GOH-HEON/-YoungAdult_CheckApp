        ...
        <aside className="space-y-4 self-start rounded-2xl border border-slate-200 bg-white p-5 shadow-sm xl:sticky xl:top-28">
          <div className="flex items-center justify-between gap-3">
            ...
            <Link
              ...
              className={[
                ...
                "hover:bg-[#1d4ed8] hover:!text-white hover:shadow-md active:translate-y-[1px] active:scale-[0.98]",
                selectedDate === todayDate ? "ring-2 ring-[#2563eb]/15" : "",
              ].join(" ")}
            >
              새 기록
            </Link>
          ...
          <form action={deleteLeadershipMeetingsAction} className="space-y-3">
            ...
            {((recentMeetings as LeadershipMeetingRow[] | null) ?? []).map((meeting) => {
              ...
              const active = selectedDate === meeting.meeting_date;

              return (
                <div
                  key={meeting.id}
                  className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-4 transition hover:border-slate-300 hover:bg-slate-50"
                >
                  <label className="mt-1 inline-flex shrink-0 items-center">
                    <input
                      ...
                      aria-label={`${formatDate(meeting.meeting_date)} 임원모임 삭제`}
                      disabled={!canManage}
                    />
                  </label>
                  <Link
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="truncate text-base font-bold">{formatDate(meeting.meeting_date)} 임원모임</p>
                        <p className="mt-1 truncate text-xs font-semibold text-slate-500">{meeting.title}</p>
                      </div>
                      <span
                        className={[
                          "shrink-0 rounded-full px-3 py-1.5 text-xs font-bold",
                          active ? "bg-white text-[#2563eb]" : "bg-slate-100 text-slate-500",
                        ].join(" ")}
                      >
                    ...
                    <p className="mt-3 text-xs font-semibold text-[#2563eb]">선택해서 불러오기</p>
                  </Link>
                </div>
              );
            })}
              ...
              <div className="rounded-lg bg-slate-50 px-4 py-3 text-sm text-slate-600">
                <p className="font-semibold text-slate-900">
                  {currentMeeting
                    ...
                    : `${formatDate(selectedDate)} 회의는 아직 기록이 없습니다.`}
                </p>
                <p className="mt-1 text-xs text-slate-500">저장된 기록 메뉴에서 항목을 선택하면 해당 회차가 불러와집니다.</p>
              </div>
